...
public class FBOLightMaskGenerator implements LightMaskGenerator
{
	...
	public void generateMasks(WorldCamera camera, Iterable<LightEmitter> emitters, LightMap lightMap, float topBorder)
	{
		...
		int pageX = floor(minX / LIGHT_MAP_PAGE_SIZE);
		int pageY = floor(minY / LIGHT_MAP_PAGE_SIZE);
		...
		builderShader.setUniformf("u_backmaskScale", tmpVec2.scl(1f / LIGHT_MAP_PAGE_SIZE));
		builderShader.setUniformf("u_backmaskOffset", lightMap.getPageOffset(minX, minY).scl(1f / LIGHT_MAP_PAGE_SIZE));
