package net.jumpai.world.component.size;

import com.badlogic.gdx.math.Vector2;
import net.jumpai.world.component.location.Localizable;

import static net.jumpai.util.math.MathUtil.boxCollidesBox;
import static net.jumpai.util.math.MathUtil.inBox;
...
public interface Sized extends Localizable
{
	...
	default boolean contains(float x, float y) {
		return inBox(x, y, getX() + getOrigin().x, getY() + getOrigin().y, getWidth(), getHeight());
	}
	...
	default boolean intersects(float x, float y, float width, float height) {
		return boxCollidesBox(x, y, width, height, getX() + getOrigin().x, getY() + getOrigin().y, getWidth(), getHeight());
	}
