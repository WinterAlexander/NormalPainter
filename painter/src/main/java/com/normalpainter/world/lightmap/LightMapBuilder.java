...
import com.normalpainter.world.camera.WorldBoundaries;
import com.normalpainter.world.component.light.LightObstructor;
	...
	protected LightMap build(LightMap output)
	{
		...
		for(WorldObject object : world.getObjects())
		{
			if(object instanceof LightObstructor)
			{
				...
				float halfStep = LightMap.LIGHT_MAP_TILE_SIZE / 2f;
				...
				int obsTileXStart = round(obs.getObsStartX() / LightMap.LIGHT_MAP_TILE_SIZE);
				int obsTileXEnd = round(obs.getObsEndX() / LightMap.LIGHT_MAP_TILE_SIZE);

				int obsTileYStart = round(obs.getObsStartY() / LightMap.LIGHT_MAP_TILE_SIZE);
				int obsTileYEnd = round(obs.getObsEndY() / LightMap.LIGHT_MAP_TILE_SIZE);

				for(int x = obsTileXStart; x < obsTileXEnd; x++)
					for(int y = obsTileYStart; y < obsTileYEnd; y++)
						if(obs.blocks(tmpVector2.set(x * LightMap.LIGHT_MAP_TILE_SIZE + halfStep, y * LightMap.LIGHT_MAP_TILE_SIZE + halfStep)))
						{
		...
		int minX = round(boundaries.getMin().x / LightMap.LIGHT_MAP_TILE_SIZE);
		int maxX = round(boundaries.getMax().x / LightMap.LIGHT_MAP_TILE_SIZE);
		int minY = round(boundaries.getMin().y / LightMap.LIGHT_MAP_TILE_SIZE);
		int maxY = round(boundaries.getMax().y / LightMap.LIGHT_MAP_TILE_SIZE);
