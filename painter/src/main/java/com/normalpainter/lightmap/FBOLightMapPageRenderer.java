...
public class FBOLightMapPageRenderer implements LightMapPageRenderer
{
	...
	public FBOLightMapPageRenderer(ShaderProgram radialShader, ShaderProgram defaultShader, Texture dummyRegion)
	{
		...
		cam.setToOrtho(true, LightMap.LIGHT_MAP_PAGE_TILE_COUNT + PADDING * 2, LightMap.LIGHT_MAP_PAGE_TILE_COUNT + PADDING * 2);
		cam.update();
	...
	public Texture render(LightMap map, int pageX, int pageY, float topBorder)
	{
		...
		FrameBuffer fbo = new YieldingFrameBuffer(Format.RGBA8888,
				LightMap.LIGHT_MAP_PAGE_TILE_COUNT + PADDING * 2,
				LightMap.LIGHT_MAP_PAGE_TILE_COUNT + PADDING * 2,
				false);
		...
		int end = LightMap.LIGHT_MAP_PAGE_TILE_COUNT + CIRCLE_SIZE / 2 + PADDING;
		...
		for(int x = start; x < end; x++)
			for(int y = start; y < end; y++)
			{
				...
				if(x < 0)
					curPageX = pageX - 1;
				else if(x >= LightMap.LIGHT_MAP_PAGE_TILE_COUNT)
					curPageX = pageX + 1;
				else
					curPageX = pageX;
				...
				if(y < 0)
					curPageY = pageY - 1;
				else if(y >= LightMap.LIGHT_MAP_PAGE_TILE_COUNT)
					curPageY = pageY + 1;
				else
					curPageY = pageY;
				...
				int inPageX = Math.floorMod(x, LightMap.LIGHT_MAP_PAGE_TILE_COUNT);
				int inPageY = Math.floorMod(y, LightMap.LIGHT_MAP_PAGE_TILE_COUNT);

				float posY = pageY * LightMap.LIGHT_MAP_PAGE_SIZE + y * LightMap.LIGHT_MAP_TILE_SIZE;
