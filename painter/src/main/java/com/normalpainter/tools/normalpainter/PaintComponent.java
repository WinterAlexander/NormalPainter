...
import com.normalpainter.tools.normalpainter.buffer.GdxPixmap;
import com.normalpainter.tools.normalpainter.buffer.PixmapBuffer;
import com.normalpainter.tools.normalpainter.buffer.RangedGdxBuffer;
import com.normalpainter.tools.normalpainter.jpun.JPenListener;

import static com.badlogic.gdx.math.MathUtils.clamp;
import static java.lang.Math.abs;
import static java.lang.Math.max;
import static java.lang.Math.min;
import static com.normalpainter.tools.normalpainter.NormalPainterScreen.WORLD_PERFECT_SCALE;
...
public class PaintComponent implements JPenListener
{
	...
	public boolean fillLine(int prevPixX, int prevPixY, int pixelX, int pixelY)
	{
		...
		if(minX - radius + 1 >= pixmap.getWidth()
				|| minY - radius + 1 >= pixmap.getHeight()
				|| maxX + radius <= 0
				|| maxY + radius <= 0)
			return false;
	...
	{
		if(pixelX - radius + 1 >= pixmap.getWidth()
				|| pixelY - radius + 1 >= pixmap.getHeight()
				|| pixelX + radius <= 0
				|| pixelY + radius <= 0)
			return false;
	...
	private void drawPixel(int x, int y, float rad, int brushSize)
	{
		...
		float opacity = brushSize == hardRad ? 1f : clamp((brushSize - rad) / (brushSize - hardRad), 0f, 1f);
	...
	public void updatePreview(boolean clean)
	{
		if(clean)
			preview.fullRange();
		if(clean || startX == -1)
			preview.drawPixmap(pixmap,
					preview.getMinX(), preview.getMinY(),
					preview.getMinX(), preview.getMinY(),
					preview.getMaxX() - preview.getMinX(), preview.getMaxY() - preview.getMinY());
		else
			preview.drawPixmap(pixmap, startX, startY, startX, startY, endX - startX, endY - startY);
		preview.initRange();
		...
		if(maskMultiply)
		{
			...
			{
				maskPreview.drawPixmap(maskBuffer, startX, startY, startX, startY, endX - startX, endY - startY);
	...
	{
		renderBuffer(pixmap, brushBuffer.getMinX(), brushBuffer.getMinY(), brushBuffer.getMaxX(), brushBuffer.getMaxY());
	}
	...
	{
		flushMaskBuffer(buffer, destination, clear, buffer.getMinX(), buffer.getMinY(), buffer.getMaxX(), buffer.getMaxY());
	}

	public void flushMaskBuffer(RangedGdxBuffer buffer, Pixmap destination, boolean clear, int startX, int startY, int endX, int endY)
	{
		for(int x = startX; x < endX; x++)
			for(int y = startY; y < endY; y++)
			{
				...
				bufferColor.r = bufferColor.r * bufferColor.a + otherColor.r * otherColor.a * (1f - bufferColor.a);
				bufferColor.g = bufferColor.g * bufferColor.a + otherColor.g * otherColor.a * (1f - bufferColor.a);
				bufferColor.b = bufferColor.b * bufferColor.a + otherColor.b * otherColor.a * (1f - bufferColor.a);
	...
	public void init()
	{
		screen.buffer = new GdxPixmap(screen.painter.pixmap.getWidth(), screen.painter.pixmap.getHeight());
